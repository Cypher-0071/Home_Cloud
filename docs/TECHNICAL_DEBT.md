...
### 12. Local LAN Auto-Switching Blocked by Middleware Order
* **Debt**: In `agent/index.js`, `app.use("/api", authMiddleware)` is declared before `app.get("/api/health")`.
* **Tradeoff**: Cross-origin micro-pings to `http://<local-ip>:3000/api/health` from `useNetworkDetector.ts` arrive without authentication cookies and receive `401 Unauthorized`. This prevents the client from confirming local connectivity and auto-upgrading from the Cloudflare Tunnel to the faster local Wi-Fi LAN.

### 13. Unhandled Process Pipe Errors in File Search (`fdfind` / `fzf`)
...
### 14. Password Equality Edge Case in Auth Route
...
### 15. Named Docker Volumes Blocked by Path Jailing
...
### 16. Docker Compose Engine Hybridization & V2 Spec Gaps
...
### 17. Ingress YAML Autowiring Race Condition & Concurrency Locking
...
### 18. One-Way Form-to-YAML Synchronization in Deploy Modal
...
### 19. File Explorer Breadcrumb Overflow on Deep Paths
...
### 20. System Monitor Historical Data Loss on Window Unmount
